...
def main():
    ...
    if st.button("Optimize", type="primary"):
        ...
        with col2:
            ...
            # Token Savings
            st.metric(
                "Token Reduction", f"{token_savings} tokens", f"{percent_savings:.1f}%"
            )

            # Cost Savings
            st.metric(
                "Estimated Cost Savings", f"${total_cost_savings:.4f}", f"per call"
            )

            # Visual indicator
            st.progress(percent_savings / 100)
            st.caption(f"Reduced to {100-percent_savings:.1f}% of original size")
